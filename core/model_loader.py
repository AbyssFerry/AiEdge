...
def create_model_settings_from_list():
    """根据 models.txt 中的模型列表创建 ModelSettings"""
    models_list = read_models_list()
...
def add_model_to_list(repo_id: str, filename: str):
    """添加模型到 models.txt"""
    # 先读取现有模型，避免重复
    existing_models = read_models_list()
...
def remove_model_from_list(filename: str):
    """从 models.txt 删除模型"""
    if not config.MODELS_TXT.exists():
        return
